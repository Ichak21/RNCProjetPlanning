...
def main(quantite_a_produire):
    ...
    # Reste du code du modèle (tel qu'il est dans l'exemple initial)
    # ...
    ...
    result = {
        "QTY": str(RFC.predict_proba(X_prepro_flat_qty)[0][1]),
        "KTE": str(RFC_KE.predict_proba(X_prepro_flat_ke)[0][1]),
        "Planning": postes_operateurs,
        "Presence_SST": str(SST_ONJOB).lower()
    }

    # Convert the "Planning" dictionary values to the desired format
    for key, value in result["Planning"].items():
        if value:
            result["Planning"][key] = [value[0], value[1].tolist()]
        else:
            result["Planning"][key] = value

    # print(result)
    return result

    # Enregistrement dans un fichier JSON
    # with open('model_resultat2.json', 'w') as json_file:
    #     json.dump(result, json_file)
    


# if __name__ == "__main__":
#     parser = argparse.ArgumentParser(description='Process quantity to produce.')
#     parser.add_argument('quantity', type=int, help='The quantity to produce.')
